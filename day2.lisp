...
(defun get-list-of-nums (str)
  (map 'list
       #'parse-integer
       (split-sequence:split-sequence-if #'(lambda (x) (find x '(#\space #\tab)))
                                         str
                                         :remove-empty-subseqs t)))

(defun single-row-checksum (str)
  (let* ((nums (get-list-of-nums str))
         (max (apply #'max nums))
         (min (apply #'min nums)))
    (- max min)))
...
(defun find-divisble-pair-difference-in-list (s)
  (abs (apply #'my-divide
              (car (remove-if-not #'(lambda (x) (or (= (rem (car x) (cadr x) )  0)
                                                    (= (rem (cadr x) (car x)) 0)))
                                  (loop for x in s
                                        nconc (loop for y in (cdr (member x s))
                                                    collect (list x y))))))))


(defun day-2-2 (str)
  (apply #'+ (map 'list
                  #'(lambda (str) (find-divisble-pair-difference-in-list (get-list-of-nums str)))
                  (split-sequence:split-sequence #\linefeed *test-string* :remove-empty-subseqs t))))
