...
(defun cleanup-test-string (str)
  (arrow-macros:as-> str s
...
; neede for part 2
(defparameter *weight-lookup* ())
(defparameter *test* ())

(defun day-7-1 ()
  (let* ((new-str (cleanup-test-string *test-string*))
         (my-list (with-input-from-string (s new-str)
                    (loop for line = (read-line s nil)
                          while line
                          collect (split-sequence:split-sequence #\space line :remove-empty-subseqs t))))
         (test (map 'list #'(lambda (x) (cons (car x) (list (cddr x)))) my-list))
         (test (map 'list #'(lambda (x) (cons (car x) (list (cddr x)))) my-list))
         (weight-lookup (map 'list #'(lambda (x) (cons (car x) (list (parse-integer (cadr x))))) my-list)))
    (setf *weight-lookup* weight-lookup)
    (setf *test* test)
    (loop
      (when (= 1 (length test)) (return (car test)))
      (loop for r in (mapcar #'car (remove-if #'(lambda (x) (car (cdr x))) test))
            do (progn
                 (setf test (remove r test :key #'car))
                 (loop for li in test
                       do (setf (cadr li) (remove r (cadr li) :test #'equal)))
                 )))))

; needed for part 2
(defparameter root (car (day-7-1)))

(defun is-balanced (xs)
  (apply #'= xs))

(defun get-weight-node (n)
  (cadr (find n *weight-lookup* :key #'car :test #'equal)))

(defun get-imbalance (root)
  (let* ((my-weight (get-weight-node root))
        (my-children (cadr (find root *test* :key #'car :test #'equal)))
        (children-weight (map 'list #'get-imbalance my-children))
         (immidiate-children-weight (map 'list #'get-weight-node my-children))
         (children-weight-sum (apply #'+ children-weight))
         (weight-to-return (+ my-weight children-weight-sum))
         )
    (print root)
    (print weight-to-return)
    (if *unbalanced* 
        weight-to-return
        (if (not children-weight)
            (progn (print weight-to-return) weight-to-return)
            (if (is-balanced children-weight)
                weight-to-return
                (progn (setf *unbalanced* (list root my-children children-weight immidiate-children-weight)) weight-to-return)
            )
            ))))
