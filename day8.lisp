...
(defparameter *accepted-operators-map* '(("<" . <) (">" . >)  ("<=" . <=) (">=" . >=) ("!=" . not-eq) ("==" . =)))

(defun handle-single-line (l)
  (let* ((tokens (split-sequence:split-sequence #\space l))
         (fun (if (string= (nth 1 tokens) "inc")
                  '+
                  '-))
         (condition-fn (cdr (assoc (nth 5 tokens) *accepted-operators-map* :test #'equal)))
         (conditional-register (nth 4 tokens))
         (is-condition-true (funcall condition-fn (gethash (nth 4 tokens) *register-hashmap* 0) (parse-integer (nth 6 tokens))))
         (new-register-value (funcall fun (gethash (nth 0 tokens) *register-hashmap* 0) (parse-integer (nth 2 tokens)))))
    (when is-condition-true (setf (gethash (nth 0 tokens) *register-hashmap* 0) new-register-value))
    (when (< *max* (gethash (nth 0 tokens) *register-hashmap* 0)) (setf *max* (gethash (nth 0 tokens) *register-hashmap*)))))

(defun day-8-1-2 ()
  (with-input-from-string (s *test-string*)
    (defparameter *register-hashmap* (make-hash-table :test 'equal))
    (defparameter *max* 0)
    (loop for line = (read-line s 'nil)
          while line
          do (progn
               (print line)
               (print (handle-single-line line)))))
  (let ((ans (apply #'max (loop for key being the hash-keys of *register-hashmap*
                                collect (gethash key *register-hashmap*))))
        (max *max*))
    (list ans max)))
